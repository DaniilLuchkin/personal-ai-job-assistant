...
export class HeuristicProvider implements LLMProvider {
  ...
  async adaptResume(_job: Job, _resume: Resume, _profile: UserProfile): Promise<ResumeAdaptation> { throw new Error('Resume adaptation requires OpenRouter. Configure the Oracle LLM gateway or a local OpenRouter key in Settings.'); }
  async generateFieldAnswer(input: Parameters<LLMProvider['generateFieldAnswer']>[0]) {
    ...
    if (/salary|compensation|pay expectation/i.test(input.fieldLabel)) {
      const salaryPreference = p.preferences.find((item) => /salary|compensation|pay|cad|usd|\$/i.test(item));
      if (salaryPreference) return salaryPreference;
      throw new Error('No salary preference is saved. Enter the answer manually, then save it as reusable knowledge.');
    }
    if (/availability|start date|notice period|when can you start/i.test(input.fieldLabel)) {
