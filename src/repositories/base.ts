...
export class Repository<T extends { id: string }> {
  constructor(private readonly store: 'resumes' | 'jobs' | 'sessions' | 'fields' | 'fieldRules' | 'knowledge' | 'applications') {}
  ...
  async clear(): Promise<void> { await (await dbPromise).clear(this.store); }
}
