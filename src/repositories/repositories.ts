...
import type { Job, JobSession, FieldRule, FormField, KnowledgeItem, Resume, Settings, UserProfile, ApplicationRecord } from '../types/models';
...
export class FieldRuleRepository extends Repository<FieldRule> { constructor() { super('fieldRules'); } }
...
export class SettingsRepository {
  ...
  async clear(): Promise<void> { await (await dbPromise).clear('settings'); }
}
export class UserProfileRepository {
  ...
  async clear(): Promise<void> { await (await dbPromise).clear('profile'); }
}

export const repos = { resumes: new ResumeRepository(), jobs: new JobRepository(), sessions: new SessionRepository(), fields: new FormFieldRepository(), fieldRules: new FieldRuleRepository(), knowledge: new KnowledgeRepository(), applications: new ApplicationRepository(), settings: new SettingsRepository(), profile: new UserProfileRepository() };
