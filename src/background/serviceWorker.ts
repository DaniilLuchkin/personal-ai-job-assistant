...
const configureParserAlarm = async () => {
  ...
  if (!settings?.parserEnabled || !settings.apifyApiKey || !settings.apifyActor) return;
  ...
  const [result] = await chrome.scripting.executeScript({
    ...
    func: (): FormField[] => {
      ...
      const labelFor = (element: Element) => {
        ...
        return (explicit || labelledBy || element.getAttribute("aria-label") || parent || fieldset || element.getAttribute("placeholder") || sibling || element.getAttribute("name") || "Unnamed field").replace(/\s+/g, " ").trim();
      };
...
const fillFieldsDirectly = async (
  tabId: number,
  fields: Array<{ selector: string; value: string; checked?: boolean }>,
): Promise<ExtensionResponse> => {
  const [result] = await chrome.scripting.executeScript({
    target: { tabId, frameIds: [0] },
    args: [fields],
    func: (items: Array<{ selector: string; value: string; checked?: boolean }>) => {
      let filled = 0;
      for (const item of items) {
        ...
        if (element instanceof HTMLInputElement && (element.type === "checkbox" || element.type === "radio")) {
          element.checked = item.checked ?? /^(true|1|yes|on)$/i.test(item.value);
        ...
        try {
          ...
          try {
            ...
            if (message.type === "DETECT_FORM_FIELDS") {
              try {
                sendResponse(await detectFieldsDirectly(tabId));
                return;
              } catch {
                // Continue with a user-facing error.
              }
            ...
            if (message.type === "FILL_FORM_FIELDS") {
              try {
                sendResponse(await fillFieldsDirectly(tabId, message.fields));
                return;
              } catch {
                // Continue with a user-facing error.
              }
